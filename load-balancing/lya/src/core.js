...
const lyaStartUp = (callerRequire, lyaConfig) => {
  ...
  const createHandler = (moduleClass) => {
    return {
      apply: function(target, thisArg, argumentsList) {
        ...
        const package = packageName.get(target);
  ...
  const setNamePath = (type, moduleExports, importName) => {
    ...
    packageName.set(moduleExports, importName);
  ...
  const namePathSet = (key, name, path, package) => {
    objectName.set(key, name);
    objectPath.set(key, path);
    packageName.set(key, package);
  };
      ...
      if (exportType !== 'undefined' && target[name] != null &&
        ...
        if (exportType === 'object') {
          ...
          if (Object.entries(target[name]).length) {
            ...
            const package = packageName.get(receiver) ?
                packageName.get(receiver) : packageName.get(target);
            hookCheck(policy.onRead, {
              target: target,
              name: name,
            ...
            namePathSet(currObject, childName, birthplace, package);
          ...
          if (!withProxy.has(target[name])) {
            ...
            namePathSet(currFunction, parentName, currModule, package);
            hookCheck(policy.onRead, {
              target: target,
              name: name,
              nameToStore: nameToStore,
            ...
            namePathSet(key, parentName, currModule, package);
          }
